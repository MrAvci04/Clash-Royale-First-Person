...
Week 14: 8.5 hours
	...
	-1 hour: trying to make health bar for red player , the problem was in the image (i dont know what)
			 but the solution was to change the textture type to 2D and UI instead of Default.
	...
	-20 minutes: adding {Q,E,U,O} to the buttons and Canvas 
			(BPLayer shoots with E , RPlayer with O .. BPlayer hidesthe weapons with Q , RPlayer with U}.
...
Week 15: 8 hours
	...
	-2 hours : cards interaction on Canvas for RPlayer , with texts.

	-1 hour : adding it for BPlayer with changing the relevant field , and fixing bugs in the script -buttons-

	-2 hour : connecting the sword with the Skeleton ,watching videos about Layers ,
		hiding relevant character from cameras using Layers ,and Player's shape becomes skeleton when he chooses the skeleton card.

	-1 hour : looking for Wizard Model , downloading and importing in unity , and connecting with the old hand , interacting with the wizar card.

	-1 hour : fixing the scripts after adding 3d wizard character to support interactions, and enhancing the UI.

	-1 hour : adding health canvas for the castle and testing all the new features.
